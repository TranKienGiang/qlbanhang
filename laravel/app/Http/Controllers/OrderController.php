...
class OrderController extends Controller
{
    ...
    public function purchase(Request $request)
    {
        ...
        foreach($dataOrderProduct['orderProductId'] as $keyid => $product_id){
            $checkProduct = Product::find($product_id);
            $productQuantity = $checkProduct->quantity;
            $soldProduct = $checkProduct->sold_quantity;
            foreach($dataOrderProduct['orderQuantity'] as $keyquantity => $quantityPd){
                if($keyid == $keyquantity){
                    $product_remain = $productQuantity - $quantityPd;
                    $checkProduct ->quantity = $product_remain;
                    $checkProduct ->sold_quantity = $soldProduct + $quantityPd;
                    $checkProduct->save();
                }
            }
    ...
    public function checkoutvnpay()
    {
        ...
        return view('viewfashion.checkoutvnpay', [
            'products' => $products,
            'productData' => $productData,
            'subtotal' => $subtotal,
            'delivery' => $delivery,
    ...
    public function purchasevnpay(Request $request)
    {
        $input = $request->only([
            'name',
            'phone',
